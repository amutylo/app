...
handlers._checks.put = function (data, callback) {
    ...
    if (update.id) {
        ...
        if (update.protocol || update.url || update.method || update.sucessCodes || update.timeoutSeconds) {
            ...
            _data.read('checks', update.id, function(err, checkData) {
                ...
                handlers._tokens.verifyToken(token, checkData.userPhone, function (tokenIsValid) {
                    if (tokenIsValid) {
                        ...
                        _data.update('checks', update.id, checkData, function (err) {
                            if (!err) {
                                callback(200, checkData)
                            }
                            else {
                                callback(500, {'Error': 'Could not update the check with id: ' + update.id});
                            }
                        })
                    }
                    else {
                    ...
                    callback(400, {'Error': 'Check id: ' + update.id + ' does not exist'})
                }
            });
        }
        else {
            callback(404, {'Error': 'Missing one of the optional fields: url or method or successCode or timeoutSeconds'});
        }
    }
    else {
    ...
    if (id) {
        ...
        _data.read('checks', id, function (err, checkData) {
            if (!err && checkData) {
                                        ...
                                        if (checkPos !== -1) {
                                            ...
                                            _data.update('users', userData.phone, userData, function (err) {
                                                if (!err) {
                                                    ...
                                                    callback(500, {'Error': 'Could not update the user with news user data.'})
                                                }
                                            }) 
                                        }   
                                        else {
                                            callback(500, {'Error': 'Could not find user check to remove.'});
                                        } 
                ...
                callback(400, {'Error': 'Check with an id: ' + id + ' does not exist.'})
            }
        });
    }   
    else {
